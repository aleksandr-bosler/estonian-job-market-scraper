...
Reads cvkeskus_jobs.json, fills full_text for image-only jobs, saves back to cvkeskus_jobs.json.
"""
...
JOBS_FILE = DATA_DIR / "cvkeskus_jobs.json"
...
def main():
    ...
    unknown_jobs = [j for j in jobs if j.get("content_type") == "unknown" and j.get("images")]
    log.info(f"Found {len(unknown_jobs)} unknown jobs with images")

    if MAX_JOBS:
        unknown_jobs = unknown_jobs[:MAX_JOBS]
    ...
    for i, job in enumerate(unknown_jobs):
        job_id = job["id"]
        images = job["images"]
        log.info(f"[{i+1}/{len(unknown_jobs)}] {job_id} - {job['title']}")
    ...
    log.info(f"Remaining unknown: {sum(1 for j in jobs_by_id.values() if j.get('content_type') == 'unknown')}")
